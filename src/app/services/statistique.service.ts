...
export class StatistiqueService {
  ...
  getStatistiques$(): Observable<Statistique[]> {
    return this.afs.collection<Statistique>('statistiques', ref => ref.orderBy("nom")).snapshotChanges().map(actions => {
  ...
  getStatistiques(): Observable<Statistique[]> {
    return this.http.get(this.url).map((res: Statistique[]) => res);
  }

  getStatistique(id: string): Observable<Statistique> {
    return this.http.get(this.url + id).map((res: Statistique) => res);
  }

  addStatistique(statistique: Statistique): Observable<Statistique> {
    return this.http.post(this.url, statistique.saveState()).map((res: Statistique) => res);
  }

  updateStatistique(statistique: Statistique): Observable<Statistique> {
    return this.http.put(this.url, { id: statistique.id, data: statistique.saveState() }).map((res: Statistique) => res);
  }
